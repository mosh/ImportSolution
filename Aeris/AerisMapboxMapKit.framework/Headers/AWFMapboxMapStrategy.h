...
/**
 *  An `AWFMapboxMapStrategy` object implements the `AWFMapStrategy` protocol and provides the necessary functionality for managing a `AWFWeatherMap` configured
 *  with an `MGLMapView` instance and Mapbox base map.
 */
...
/**
 ...
 Setting this value does not change the primary `MGLMapViewDelegate` associated with the map view. Delegate events are forwarded to the delegate
 assigned by this value. The strategy must remain the only delegate to the map view assocated with this strategy.
 */
@property (nonatomic, unsafe_unretained, nullable) id<MGLMapViewDelegate> mapViewDelegate;
