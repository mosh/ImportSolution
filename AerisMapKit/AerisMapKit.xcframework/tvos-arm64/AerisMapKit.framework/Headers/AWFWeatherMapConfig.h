...
/// A Boolean value indicating whether existing animation data should be removed from the map and replaced by the latest data when the animation
/// is stopped. This option pertains primarily with raster data, such as image and tile overlays. Default value is `NO`.
///
/// Note that existin animation data will still be removed and reset whenever the visible region of the map changes regardless of the value for this
/// property.
@property (nonatomic, assign) BOOL shouldRemoveAnimationDataWhenStopped;
