...
@protocol AWFWeatherMapView <NSObject>
 ...
 */
@property (readonly, nonatomic, strong) UIView<AWFWeatherMapView> *mapView;

/**
 An object that needs to receive all delegate messages from the specific mapping SDK being used.
 */
@property (nullable, nonatomic, weak) id mapViewDelegate;
 ...
 */
@property (readonly, nonatomic) NSArray<NSString *> *activeLayerTypeCodes;
 ...
 */
@property (nullable, nonatomic, weak) id<AWFWeatherMapDelegate> delegate;
 ...
 */
@property (nullable, nonatomic, weak) id<AWFWeatherMapDataSource> dataSource;
...
/// Starts loading data required by the animation, if any. Loading will be done in the background, meaning map content will not
/// show the loading process nor start animating once all loading has been completed.
